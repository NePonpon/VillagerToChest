...
execute if score #SettingChest_Count VillagerToChest matches 2.. as @e[tag=SettingChest] run tellraw @p [{"text": "設定できるチェストは1つまでです！","color": "green","bold": true}]
execute if score #SettingChest_Count VillagerToChest matches 2.. run kill @e[tag=SettingChest]
